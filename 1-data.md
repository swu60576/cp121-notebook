...
* เอา `string + number` (หรือสลับที่กัน) นิพจน์นั้นก็จะมี type เป็น string ค่าก็พวกตัวอักษร
* เอา `number (>, <, <=, =>, ==, !=) number` นิพจน์นั้นก็จะมี type เป็น boolean (`true`, `false`) ค่าก็พวก จริง, เท็จ
* เอา `boolean (&&, ||) boolean` นิพจน์นั้นก็จะมีค่าเป็นค่าความจริง และมี type เป็น boolean (`true`, `false`)
...
// 🚫 condition: 2 + 5, 10 % 5 (นิพจน์ที่ type ไม่ใช่ boolean (ถึงแม้ y + 5, 10 % 5 จะให้ผลเป็น type number ซึ่งแปลงเป็น boolean ได้ แต่เราก็ไม่ได้ใช้ if-else เพื่ออะไรแบบนี้ใช่ไหม))
...
//เรื่องของการเข้าถึง สมมุติว่าเราเข้าถึง 2d array แบบเดียวกับ 1d ก็ผลที่ออกมาก็จะเป็น
print(mt2[0])
print(mt2[1])
...
print('-- end mtx --')
```
ทั้ง 2d array และ 1d array ต่างก็ต้องระวังเรื่องการเข้าถึง index ที่ไม่มีอยู่ เหมือนกัน <br >
และโน๊ตไว้สักนิดว่า error การ loop หลุด array ของ js อาจดูไม่มีผลร้ายแรง แต่ถ้าเป็นภาษาอื่น (เช่น c, java) ล่ะก็... 🙂
## Object 
